...
fn tree_traversal(node:Option<Box<Node>>, low:i32,high:i32,mut output:&mut Vec<i32>){
    if !node.is_none(){
        ...
        if node.as_ref().unwrap().val >= low {
            // traverse left
            tree_traversal(node.as_ref().unwrap().left.clone(), low, high,&mut output);
        }
    
        if node.as_ref().unwrap().val <= high {
            // traverse right
            tree_traversal(node.as_ref().unwrap().right.clone(), low, high, &mut output);
        }
...
fn products_in_range(root:Node,low:i32,high:i32)->Vec<i32>{
    ...
    tree_traversal(Some(Box::new(root)), low, high, &mut output);
    output
...
fn main() {
    ...
    let mut binarySearchTree:BinarySearchTree = BinarySearchTree{
        root: Node::new(product_prices[0])
    };

    for i in 1..product_prices.len(){
        binarySearchTree.root.insert(product_prices[i]);
    }

    println!("{:?}",binarySearchTree);

    let range_prices = products_in_range(binarySearchTree.root,7,20);
    println!("products in price ranges: {:?}",range_prices);
